...
!!! Seealso "About the 4C's Security Model"
    ...
    has taken with security in EDB Postgres for Kubernetes.

## Code

Source code of EDB Postgres for Kubernetes is *systematically scanned* for static analysis purposes,
...
!!! Important
    ...
    for the entire delivery of EDB Postgres for Kubernetes, meaning that each commit is validated
    against all the linters defined by GolangCI-Lint.

## Container

Every container image that is part of EDB Postgres for Kubernetes is automatically built via CI/CD pipelines following every commit.
...
!!! Seealso "About the Container level security"
    Please refer to ["Security and Containers in EDB Postgres for Kubernetes"](https://www.enterprisedb.com/blog/security-and-containers-cloud-native-postgresql)
    blog article for more information about the approach that EDB has taken on
    security at the container level in EDB Postgres for Kubernetes.
...
!!! Important
    ...
    EDB Postgres for Kubernetes for finer control.
...
EDB Postgres for Kubernetes exposes ports at operator, instance manager and operand
