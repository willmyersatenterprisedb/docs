...
In a nutshell, a `Pooler` in EDB Postgres for Kubernetes is a deployment of
...
layer based on PgBouncer changes the architecture of EDB Postgres for Kubernetes,
...
The easiest way to explain how EDB Postgres for Kubernetes implements a PgBouncer
...
Additionally, EDB Postgres for Kubernetes automatically creates a secret with the
...
Any PgBouncer pooler is transparently integrated with EDB Postgres for Kubernetes
...
!!! Important
    ...
    [`cnp` plugin](kubectl-plugin.md#promote), and finally restoring the `paused`
    attribute to `false`.
...
specific EDB Postgres for Kubernetes cluster (a service, to be precise). It is not
