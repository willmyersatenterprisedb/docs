...
EDB Postgres for Kubernetes has been designed to natively support TLS certificates.
...
EDB Postgres for Kubernetes is very flexible when it comes to TLS certificates, and
...
1. [**operator managed**](#operator-managed-mode): certificates are internally
   ...
   by EDB Postgres for Kubernetes
2. [**user provided**](#user-provided-certificates-mode): certificates are
   ...
   secrets - EDB Postgres for Kubernetes integrates itself with cert-manager (see
   examples below)
